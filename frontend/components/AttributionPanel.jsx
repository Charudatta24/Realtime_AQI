  ...
  useEffect(() => {
    ...
    const fetchAttribution = async () => {
      ...
      try {
        ...
        const res = await fetch(`${(process.env.NEXT_PUBLIC_API_BASE || 'https://realtime-aqi-1u9g.onrender.com').replace(/\/+$/, '')}/api/metrics/attribution?city=${city}&row=${selectedCell.row}&col=${selectedCell.col}${timeParam}`);
