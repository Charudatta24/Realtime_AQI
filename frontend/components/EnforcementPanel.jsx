...
export default function EnforcementPanel({ city, onDispatchSuccess }) {
  ...
  useEffect(() => {
    if (!city) return;
    setLoadingRecs(true);
    fetch(`${(process.env.NEXT_PUBLIC_API_BASE || 'https://realtime-aqi-1u9g.onrender.com').replace(/\/+$/, '')}/api/metrics/enforcement?city=${city}`)
      .then(r => r.json())
  ...
  const handleConfirmDispatch = async (e) => {
    ...
    try {
      const res = await fetch(`${(process.env.NEXT_PUBLIC_API_BASE || 'https://realtime-aqi-1u9g.onrender.com').replace(/\/+$/, '')}/api/dispatch`, {
        method: 'POST',
        headers: {
