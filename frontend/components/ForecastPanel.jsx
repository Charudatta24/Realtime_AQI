    ...
    const fetchForecasts = async () => {
      ...
      try {
        ...
        const [res24, res48, res72] = await Promise.all([
          fetch(`${(process.env.NEXT_PUBLIC_API_BASE || 'https://realtime-aqi-1u9g.onrender.com').replace(/\/+$/, '')}/api/metrics/forecast?city=${city}&hours=24${timeParam}`),
          fetch(`${(process.env.NEXT_PUBLIC_API_BASE || 'https://realtime-aqi-1u9g.onrender.com').replace(/\/+$/, '')}/api/metrics/forecast?city=${city}&hours=48${timeParam}`),
          fetch(`${(process.env.NEXT_PUBLIC_API_BASE || 'https://realtime-aqi-1u9g.onrender.com').replace(/\/+$/, '')}/api/metrics/forecast?city=${city}&hours=72${timeParam}`)
        ]);
